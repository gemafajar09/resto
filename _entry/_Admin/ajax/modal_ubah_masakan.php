	      			...
	      			<div class="form-group">
						<div class="row">
							<div class="col-sm-6">
								...
								<input type="text" maxlength = "30" class="form-control" name="nama_masakan" id="nama_masakan" value="<?php echo $data['nama_masakan'];?>" required oninvalid="this.setCustomValidity('Nama Masakan Harus Di isi')" onkeypress="hack(event)" oninput="setCustomValidity('')">
							</div>
							...
							<div class="col-sm-6">
			                    ...
			                    ?>
			                    <option value="<?php echo $hasil['id_kategori']; ?><?php if($id_kategori=='nama_kategori') {echo 'select="select"';} ?>"><?php echo $hasil['nama_kategori']; ?></option>
			                    <?php } ?>
			                    </select>
							</div>
						</div>
					...
					<div class="form-group">
						<div class="row">
							<div class="col-sm-6">
								...
								<input type="number" maxlength="10" class="form-control" name="harga" id="harga" value="<?php echo $data['harga'];?>" oninput="javascript: if (this.value.length > this.maxLength) this.value = this.value.slice(0, this.maxLength);" required oninvalid="this.setCustomValidity('Harga Harus Di isi')" onkeypress="hack(event)" oninput="setCustomValidity('')">
							</div>
							...
							<div class="col-sm-6">
								...
								<select id="jenis" name="jenis" class="select">
									...
									<option value="Makanan <?php if($jenis=='Foods') {echo 'select="select"';} ?>">Makanan</option>
									<option value="Minuman <?php if($jenis=='Drink') {echo 'select="select"';} ?>">Minuman</option>
								</select>
							</div>
						</div>
					</div>
	        			...
	        			<div class="row">
	        				<div class="col-sm-12">
								...
								<select id="status_masakan" name="status_masakan" class="select">
									<option value="" disabled select>Pilih</option>
									<option value="Y <?php if($status_masakan=='Y') {echo 'select="select"';} ?>">Tesedia</option>
